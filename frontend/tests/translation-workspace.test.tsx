...
test('dictionary confirmed and ignored summary remains visible', () => {
  ...
  const html = renderToStaticMarkup(React.createElement(api.TranslationWorkspace));
  for (const text of ['Dictionary summary', '842 confirmed', '37 ignored', '极光石', 'Cực Quang Thạch', 'IGNORED', 'item/material classification unresolved', 'confidence 97.8%', 'fallback: provisional']) assert.ok(html.includes(text), text);
});
